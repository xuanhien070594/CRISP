...
int main() {
  /// Construct Drake MathematicalProgram
  ...
  objectiveFunction.evaluator()->set_description("quadraticObjective");
  ...
  dynamicsConstraint.evaluator()->set_description("dynamicsConstraint");
  ...
  initialStateConstraint.evaluator()->set_description("initialStateConstraint");
  finalStateConstraint.evaluator()->set_description("finalStateConstraint");

  // Add control limit constraints
  Binding<Constraint> controlLimitConstraint = prog.AddBoundingBoxConstraint(-1, 1, x(2));
  controlLimitConstraint.evaluator()->set_description("controlLimitConstraint");
  ...
  trackingProblem.parseDrakeMathematicalProgram(prog);

  /// Verify all objective and constraint functions are correct
  vector_t test_x(5);
